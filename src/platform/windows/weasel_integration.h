// src/platform/windows/weasel_integration.h
// Weasel 集成层 - 将我们的核心模块集成到 Weasel 前端

#ifndef IME_WEASEL_INTEGRATION_H
#define IME_WEASEL_INTEGRATION_H
...
// Weasel 集成配置
struct WeaselIntegrationConfig {
    std::wstring userDataPath;      // 用户数据目录
    std::wstring sharedDataPath;    // 共享数据目录
    std::wstring logPath;           // 日志目录
    bool enableCloudSync;           // 是否启用云端同步
    bool enableAutoLearn;           // 是否启用自动学词
    int pageSize;                   // 每页候选词数量

    // 默认配置
    static WeaselIntegrationConfig Default();
};

// Weasel 集成管理器
// 负责将我们的核心模块（词频管理、候选词合并、自动学词等）集成到 Weasel
...
public:
    ...
    // 禁止拷贝
    WeaselIntegration(const WeaselIntegration&) = delete;
    WeaselIntegration& operator=(const WeaselIntegration&) = delete;
    ...
    // @param config 配置
    // @return 是否成功
    bool Initialize(const WeaselIntegrationConfig& config);
...
private:
    ...
    // 初始化存储层
    bool InitializeStorage(const std::wstring& userDataPath);
...
private:
    bool initialized_;
    WeaselIntegrationConfig config_;
...
// 初始化集成层
// @param userDataPath 用户数据目录（UTF-16）
// @param sharedDataPath 共享数据目录（UTF-16）
// @return 0 成功，非 0 失败
...
#endif  // IME_WEASEL_INTEGRATION_H
